...
const TankDate = ({ mode, filters }) => {
  ...
  const getDisplayDate = () => {
    if (mode === "historical" && filters.dateRange) {
      const from = format(filters.dateRange.from, "d MMMM yyyy, HH:mm", { locale: es });
      const to = format(filters.dateRange.to, "d MMMM yyyy, HH:mm", { locale: es });
      return from === to ? from : `${from} - ${to}`;
    }
    return format(lastUpdate, "d MMMM yyyy, HH:mm", { locale: es });
  };
