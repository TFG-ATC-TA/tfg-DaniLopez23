...
export default function HorizontalTankModel2Blades(props) {
  ...
  return (
    <group {...props} dispose={null}>
      ...
      <animated.mesh geometry={nodes.Hatch.geometry} material={materials.HatchMaterial} position={[0, 3.257, 0]} scale={0.019} rotation={rotationHatch} />

      {/* Líneas que conectan el modelo con las etiquetas */}
      <Line points={[[0, 2.5, 0], [2, 3, 0]]} color="orange" lineWidth={2} />
      <Line points={[[0, 1, -1], [2, 1, -2]]} color="orange" lineWidth={2} />
      <Line points={[[0, 1, 1], [2, 1, 2]]} color="orange" lineWidth={2} />
      <Line points={[[0, 3, 0], [2, 3.5, 0]]} color="orange" lineWidth={2} />

      {/* Etiquetas de información */}
      <Html position={[2, 3, 0]}>
        <div className="label" style={{ backgroundColor: 'black', padding: '10px', borderRadius: '5px', color: 'white' }}>
          <h4>Milk Level: {props.milkQuantityData.milkQuantity}%</h4>
        </div>
      </Html>

      <Html position={[2, 1, -2]}>
        <div className="label" style={{ backgroundColor: 'black', padding: '10px', borderRadius: '5px', color: 'white' }}>
          <h4>Blade 1 RPM: {props.encoderData.value} RPM</h4>
        </div>
      </Html>

      <Html position={[2, 1, 2]}>
        <div className="label" style={{ backgroundColor: 'black', padding: '10px', borderRadius: '5px', color: 'white' }}>
          <h4>Blade 2 RPM: {props.encoderData.value} RPM</h4>
        </div>
      </Html>

      <Html position={[2, 3.5, 0]}>
        <div className="label" style={{ backgroundColor: 'black', padding: '10px', borderRadius: '5px', color: 'white' }}>
          <h4>Hatch: {props.switchStatus ? "Open" : "Closed"}</h4>
        </div>
      </Html>
    </group>
  )
